 ...
 * The user of this interface has precise control over where in the list each element is inserted.
	接口使用者需要在每个元素插入list的时候有清晰明确的控制。
 * The user can access elements by their integer index (position in the list), and search for elements in the list.<p>
	使用者在list中可以通过元素的索引访问元素和搜索元素。
 *
 * Unlike sets, lists typically allow duplicate elements.  
	不想set集合，list 通常是允许重复元素的。
 * More formally, lists typically allow pairs of elements <tt>e1</tt> and <tt>e2</tt> such that <tt>e1.equals(e2)</tt>,
	更通俗的说，list 常常允许一对元素的。例如e1.equals(e2)
 * and they typically allow multiple null elements if they allow null elements at all.  
	还有list 还允许放入多个null值元素。
 * It is not inconceivable(不可想象) that someone might wish to implement a list that prohibits(阻止) duplicates, by throwing runtime exceptions when the user attempts to insert them, 
	有人想实现一个list，通过抛出一个运行型异常来来阻止重复元素的插入 也是可以的。
 * but we expect this usage to be rare(稀有).<p>
	但我们希望这种用法很少。
 *
 * The <tt>List</tt> interface places additional stipulations(规定), beyond those  specified in the <tt>Collection</tt> interface,
	list 有除了Collection接口定义外的额外的规定。
 * on the contracts(约定) of the <tt>iterator</tt>, <tt>add</tt>, <tt>remove</tt>, <tt>equals</tt>, and <tt>hashCode</tt> methods.  
	有<tt>iterator</tt>, <tt>add</tt>, <tt>remove</tt>, <tt>equals</tt>, and <tt>hashCode</tt>这些约定。
 * Declarations for other inherited methods are also included here for convenience.<p>
	为了方便，这里包含了其他继承而来的方法的声明。
 *
 * The <tt>List</tt> interface provides four methods for positional (indexed) access to list elements. 
	list接口提供了4种通过index索引的方法来方位list元素。
 * Lists (like Java arrays) are zero based.  
	Lists(像数组一样)都是从0开始。
 * Note that these operations may execute in time proportional to the index value for some implementations (the <tt>LinkedList</tt> class, for example). 
	注意，这些操作可能与某些实现的索引值成比例地按时间执行。(例如 LinkedList)
 * Thus, iterating over the elements in a list is typically preferable(更胜一筹) to indexing through it if the caller does not know the implementation.<p>
